...
export const getCharacterPrompt = () => {
  ...
  // 生成する文章のフォーマット
  const format = '<フォーマット>会話分はJSON形式で、{ emotion: 感情, content: 応答 } の形式で返せ。感情はidle, sad, happy, angry のいずれかだ。感情や応答はユーザーの発言に基づいて生成しなさい。</フォーマット>'
